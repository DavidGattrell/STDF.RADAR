# SplitTestsRetestsGui.R
#
# $Id: SplitTestsRetestsGui.R,v 1.1 2021/08/02 00:31:44 david Exp $
#
# Tk/Tcl GUI wrapper for calling SplitTestsRetests.R
# called by TkRadar.R
#
...
#
#----------------------------------------------------------
# SplitTestsRetestsGui specific variables
#-----------------------------------------
if(!exists(".TkRadar.env")) .TkRadar.env <- new.env()

#in_name <- tclVar("")
#in_dir <- tclVar("")
assign("split_tr_out_file",tclVar(""),envir=.TkRadar.env)
assign("split_tr_use_xy_coords",tclVar(0),envir=.TkRadar.env)

# these defaults can be controlled in the .Rprofile file:
default_split_tr_use_xy_coords <- tclVar(0)		

#----------------------------------------------------
SplitTestsRetestsGui_defaults <- function() {
	tclvalue(split_tr_out_file) <- ""
#	tclvalue(in_dir) <- ""
#	tclvalue(in_name) <- as.character(tclObj(Rtdf_name))
	tclvalue(split_tr_use_xy_coords) <- tclObj(default_split_tr_use_xy_coords)
}


#----------------------------------------------------
split_tr_rtdf_browser <-function(...) {
	orig_name = paste(tclObj(split_tr_out_file),sep="",collapse=" ")
	...
	if (nchar(name)>0) {
		...
		name=sub("^.*/","",name)
		tclvalue(split_tr_out_file) <- name
...
#----------------------------------------------------
run_SplitTestsRetests <-function(done=FALSE,...) {

	in_file_ <- paste(tclObj(Rtdf_name),sep="",collapse=" ")
	out_file_ <- paste(tclObj(split_tr_out_file),sep="",collapse=" ")
	in_dir_ <- paste(tclObj(Rtdf_dir),sep="",collapse=" ")
	output_dir <- paste(tclObj(Rtdfs_dir),sep="",collapse=" ")
	use_xy <- as.logical(tclObj(split_tr_use_xy_coords))
	...
	my_expr = substitute(
		SplitTestsRetests(in_file=in_file_,out_file=out_file_,
...
#----------------------------------------------------
SplitTestsRetestsGui <- function() {

	SplitTestsRetestsGui_defaults()		# initialize variables...
	splittestsretests_win <- tktoplevel()
	assign("splittestsretests_win",splittestsretests_win,envir=.TkRadar.wins)
	tkwm.title(splittestsretests_win, "SplitTestsRetests")
		
	bottom_row <- tkframe(splittestsretests_win)
	default_button <- tkbutton(bottom_row,
						text="DEFAULTS",
						#anchor="w",
						width=12,
						command=SplitTestsRetestsGui_defaults)
	...
	ok_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function() run_SplitTestsRetests(done=TRUE))
	...
	cancel_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function()tkdestroy(splittestsretests_win))
	...
	apply_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function() run_SplitTestsRetests(done=FALSE))
	tkpack(apply_button,side="right")
	tkpack(bottom_row,side="bottom",anchor="w")

	dir_entry_frame <- tkframe(splittestsretests_win)
	...
	in_dir_entry_frame <- tkframe(splittestsretests_win)
	...
	in_entry_frame <- tkframe(splittestsretests_win)
	...
	out_entry_frame <- tkframe(splittestsretests_win)
	...
	out_entry <- tkentry(out_entry_frame,
						width=20,
						textvariable=split_tr_out_file)
	...
	use_xy_button <- tkcheckbutton(splittestsretests_win,
						text="use_xy_coords",
						variable=split_tr_use_xy_coords)
