# SplitSitesGui.R
#
# $Id: SplitSitesGui.R,v 1.4 2010/11/24 01:54:53 David Exp $
#
# Tk/Tcl GUI wrapper for calling SplitSites.R
# called by TkRadar.R
#
...
#
#----------------------------------------------------------
# SplitSitesGui specific variables
#-----------------------------------------
if(!exists(".TkRadar.env")) .TkRadar.env <- new.env()

assign("site_outname",tclVar(""),envir=.TkRadar.env)


#----------------------------------------------------
SplitSitesGui_defaults <- function() {
	tclvalue(site_outname) <- ""
}


#----------------------------------------------------
spl_site_out_rtdf_browser <-function(...) {
	orig_name = paste(tclObj(site_outname),sep="",collapse=" ")
	if (nchar(orig_name)<1)  orig_name="splitsites.rtdf"
...
#----------------------------------------------------
run_SplitSites <-function(done=FALSE,...) {

	in_file_ <- paste(tclObj(Rtdf_name),sep="",collapse=" ")
	out_file_ <- paste(tclObj(site_outname),sep="",collapse=" ")
	...
	my_expr = substitute(
		SplitSites(in_file=in_file_,out_file=out_file_,
						  in_dir=in_dir_)
	)
...
#----------------------------------------------------
SplitSitesGui <- function() {

	SplitSitesGui_defaults()		# initialize variables...
	splitsites_win <- tktoplevel()
	assign("splitsites_win",splitsites_win,envir=.TkRadar.wins)
	tkwm.title(splitsites_win, "SplitSites")
		
	bottom_row <- tkframe(splitsites_win)
	default_button <- tkbutton(bottom_row,
						text="DEFAULTS",
						#anchor="w",
						width=12,
						command=SplitSitesGui_defaults)
	...
	ok_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function() run_SplitSites(done=TRUE))
	...
	cancel_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function()tkdestroy(splitsites_win))
	...
	apply_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function() run_SplitSites(done=FALSE))
	tkpack(apply_button,side="right")
	tkpack(bottom_row,side="bottom",anchor="w")

	dir_entry_frame <- tkframe(splitsites_win)
	...
	in_dir_entry_frame <- tkframe(splitsites_win)
	...
	in_entry_frame <- tkframe(splitsites_win)
	...
	out_entry_frame <- tkframe(splitsites_win)
