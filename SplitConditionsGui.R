# SplitConditionsGui.R
#
# $Id: SplitConditionsGui.R,v 1.3 2010/11/24 01:53:28 David Exp $
#
# Tk/Tcl GUI wrapper for calling SplitConditions.R
# called by TkRadar.R
#
...
#
#----------------------------------------------------------
# SplitConditionsGui specific variables
#-----------------------------------------
if(!exists(".TkRadar.env")) .TkRadar.env <- new.env()

assign("cond_outname",tclVar(""),envir=.TkRadar.env)


#----------------------------------------------------
SplitConditionsGui_defaults <- function() {
	tclvalue(cond_outname) <- ""
}


#----------------------------------------------------
spl_cond_out_rtdf_browser <-function(...) {
	orig_name = paste(tclObj(cond_outname),sep="",collapse=" ")
	if (nchar(orig_name)<1)  orig_name="splitcond.rtdf"
...
#----------------------------------------------------
run_SplitConditions <-function(done=FALSE,...) {

	in_file_ <- paste(tclObj(Rtdf_name),sep="",collapse=" ")
	out_file_ <- paste(tclObj(cond_outname),sep="",collapse=" ")
	...
	my_expr = substitute(
		SplitConditions(in_file=in_file_,out_file=out_file_,
						  in_dir=in_dir_)
	)
...
#----------------------------------------------------
SplitConditionsGui <- function() {

	SplitConditionsGui_defaults()		# initialize variables...
	splitconds_win <- tktoplevel()
	assign("splitconds_win",splitconds_win,envir=.TkRadar.wins)
	tkwm.title(splitconds_win, "SplitConditions")
		
	bottom_row <- tkframe(splitconds_win)
	default_button <- tkbutton(bottom_row,
						text="DEFAULTS",
						#anchor="w",
						width=12,
						command=SplitConditionsGui_defaults)
	...
	ok_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function() run_SplitConditions(done=TRUE))
	...
	cancel_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function()tkdestroy(splitconds_win))
	...
	apply_button <- tkbutton(bottom_row,
						...
						#anchor="w",
						width=12,
						command=function() run_SplitConditions(done=FALSE))
	tkpack(apply_button,side="right")
	tkpack(bottom_row,side="bottom",anchor="w")

	dir_entry_frame <- tkframe(splitconds_win)
	...
	in_dir_entry_frame <- tkframe(splitconds_win)
	...
	in_entry_frame <- tkframe(splitconds_win)
	...
	out_entry_frame <- tkframe(splitconds_win)
