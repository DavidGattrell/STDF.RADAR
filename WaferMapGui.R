...
WaferMapGui <-function(...) {
	...
	tkbind(col_start_entry,"<KeyRelease>",function() {
					tmp <- as.numeric(tclObj(wmap_param_col_start))
	...
	tkbind(col_end_entry,"<KeyRelease>",function() {
					tmp <- as.numeric(tclObj(wmap_param_col_end))
					if( (length(tmp)>0) && is.finite(tmp) &&
							(tmp>=0.0)  && (tmp<=1.0) ) {
						tkconfigure(col_end_entry,background="white")
					} else {
	...
	# REVISIT..as far as here adding param colour customization
...
XformWaferMapGui <-function(...) {
	...
	xshift_entry <- tklabel(xshift_entry_frame,
						width=20,
						relief="sunken",
						textvariable=wmap_x_shift)
	tkpack(xshift_entry,side="left",fill="x",expand=1)
	xshift_browse <- tkbutton(xshift_entry_frame,
						text="Edit",
						command=function() integer_entry(wmap_x_shift))
	tkpack(xshift_browse,side="right")
	tkpack(xshift_entry_frame,side="top",anchor="w",fill="x")
	...
	yshift_entry <- tklabel(yshift_entry_frame,
						width=20,
						relief="sunken",
						textvariable=wmap_y_shift)
	tkpack(yshift_entry,side="left",fill="x",expand=1)
	yshift_browse <- tkbutton(yshift_entry_frame,
						text="Edit",
						command=function() integer_entry(wmap_y_shift))
	tkpack(yshift_browse,side="right")
	tkpack(yshift_entry_frame,side="top",anchor="w",fill="x")
